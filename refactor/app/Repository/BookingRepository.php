...
class BookingRepository extends BaseRepository
{
    ...
    function __construct1(Job $model, MailerInterface $mailer)
    {
        parent::__construct($model);
        $this->mailer = $mailer;
        $this->logger = new Logger('admin_logger');

        $logFilePath = storage_path('logs/admin/laravel-' . date('Y-m-d') . '.log');
    ...
    public function store($user, $data)
    {
        ...
        $data['gender'] = isset($genderMappings[$data['job_for'][0]]) ? $genderMappings[$data['job_for'][0]] : null;
        $data['certified'] = isset($certifiedMappings[$data['job_for'][1]]) ? $certifiedMappings[$data['job_for'][1]] : null;
        ...
        $data['job_type'] = isset($jobTypeMappings[$consumer_type]) ? $jobTypeMappings[$consumer_type] : null;
        ...
        if ($job->gender != null) {
            $data['job_for'][] = $genderMappings[$job->gender];
        }
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $job_ids = collect($job_ids)->filter(function ($job_id) use ($user_id) {
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        $msg_text = [
            'en' => 'Detta är en påminnelse om att du har en ' . $language . 'tolkning'
        ];

        if ($job->customer_physical_type == 'yes') {
            $msg_text["en"] .= ' (på plats i ' . $job->town . ') kl ' . $due_explode[1] . ' på ' . $due_explode[0] . ' som varar i ' . $duration . ' min. Lycka till och kom ihåg att ge feedback efter utförd tolkning!';
        } else {
            $msg_text["en"] .= ' (telefon) kl ' . $due_explode[1] . ' på ' . $due_explode[0] . ' som varar i ' . $duration . ' min. Lycka till och kom ihåg att ge feedback efter utförd tolkning!';
        }
    ...
    private function changeTranslator($currentTranslator, $data, $job)
    {
        ...
        if (!is_null($currentTranslator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if (!is_null($currentTranslator) && ((isset($data['translator']) && $currentTranslator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                ...
                $newTranslator = Translator::create($newTranslatorData);
    ...
    public function acceptJobWithId($jobId, $currentUser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($jobId, $currentUser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($currentUser->id, $jobId)) {
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    $usersArray = [$user];
                    $this->sendPushNotificationToSpecificUsers($usersArray, $jobId, $data, $msg_text, $this->isNeedToDelayPush($user->id));
                }
        ...
        if ($currentUser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->isNeedToSendPush($translator->id)) {
                    $usersArray = [$translator];
                    $this->sendPushNotificationToSpecificUsers($usersArray, $jobId, $data, $msg_text, $this->isNeedToDelayPush($translator->id)); // send Session Cancel Push to Translator
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->isNeedToSendPush($customer->id)) {
                        $usersArray = [$customer];
                        $this->sendPushNotificationToSpecificUsers($usersArray, $jobId, $data, $msg_text, $this->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
    ...
    public function alerts()
    {
        ...
        $requestData = Request::all();
        $allCustomers = DB::table('users')->where('user_type', '1')->pluck('email')->toArray();
        $allTranslators = DB::table('users')->where('user_type', '2')->pluck('email')->toArray();
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            $allJobs = Job::join('languages', 'jobs.from_language_id', '=', 'languages.id')
                ...
                ->whereIn('jobs.status', ['pending', 'on-hold'])
